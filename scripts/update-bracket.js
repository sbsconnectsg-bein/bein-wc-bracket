...
function mapMatch(match, matchNumber) {
  ...
  const penalties = score.penalties && (score.penalties.home !== null || score.penalties.away !== null)
  ...
  return {
    ...
    status: match.status,
    winner: score.winner || null, // 'HOME_TEAM' | 'AWAY_TEAM' | 'DRAW' | null — authoritative, from the API
...
function getWinnerName(m) {
  if (!m || m.status !== 'FINISHED' || !m.winner || m.winner === 'DRAW') return null;

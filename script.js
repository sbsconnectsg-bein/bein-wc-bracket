...
// Matches scripts/update-bracket.js's NOT_YET_DECIDED_STATUSES — a live or
// not-yet-started match shouldn't show winner/eliminated row styling, since
// football-data.org populates `winner` with the current leader even mid-match.
...
function matchCard(match, { roundLabel, kind } = {}) {
  ...
  const decided = !!match.winner && match.winner !== 'DRAW' && !NOT_YET_DECIDED_STATUSES.has(match.status);
...
function renderCenter(sf1, sf2, final, thirdPlace) {
  ...
  const finalDecided = !!final.winner && final.winner !== 'DRAW' && !NOT_YET_DECIDED_STATUSES.has(final.status);
  const championName = finalDecided
    ? (final.winner === 'HOME_TEAM' ? final.home.name : final.away.name)
    : null;
  const thirdDecided = !!thirdPlace.winner && thirdPlace.winner !== 'DRAW' && !NOT_YET_DECIDED_STATUSES.has(thirdPlace.status);
...
function renderTopScorers(scorers) {
  ...
  const items = scorers.slice(0, 5).map((s, i) => `
    <div class="ts-item">

...
// Flags as plain CSS color stripes rather than Unicode flag emoji — the
...
// pictograph. A CSS gradient renders identically everywhere, no font needed,
// and doubles as exactly the "country colors" accent requested.
// Each entry is a CSS linear-gradient stop list; simplified to each flag's
// 2-3 dominant colors rather than exact reproductions (e.g. emblems omitted).
const FLAG_STRIPES = {
  'Canada':      ['to right', '#D80621 20%', '#FFFFFF 20% 80%', '#D80621 80%'],
  'Morocco':     ['to bottom', '#C1272D 0% 100%'],
  'Paraguay':    ['to bottom', '#D52B1E 33%', '#FFFFFF 33% 66%', '#0038A8 66%'],
  'France':      ['to right', '#0055A4 33%', '#FFFFFF 33% 66%', '#EF4135 66%'],
  'Portugal':    ['to right', '#046A38 40%', '#DA291C 40%'],
  'Spain':       ['to bottom', '#AA151B 25%', '#F1BF00 25% 75%', '#AA151B 75%'],
  'United States': ['to bottom', '#B22234 0% 100%'],
  'USA':         ['to bottom', '#B22234 0% 100%'],
  'Belgium':     ['to right', '#000000 33%', '#FAE042 33% 66%', '#ED2939 66%'],
  'Brazil':      ['to bottom', '#009739 0% 100%'],
  'Norway':      ['to bottom', '#EF2B2D 0% 100%'],
  'Mexico':      ['to right', '#006847 33%', '#FFFFFF 33% 66%', '#CE1126 66%'],
  'England':     ['to bottom', '#FFFFFF 0% 100%'],
  'Argentina':   ['to bottom', '#75AADB 33%', '#FFFFFF 33% 66%', '#75AADB 66%'],
  'Egypt':       ['to bottom', '#CE1126 33%', '#FFFFFF 33% 66%', '#000000 66%'],
  'Switzerland': ['to bottom', '#D52B1E 0% 100%'],
  'Colombia':    ['to bottom', '#FCD116 50%', '#003893 50% 75%', '#CE1126 75%'],
};
function flagStyle(teamName) {
  const stops = FLAG_STRIPES[teamName];
  if (!stops) return 'background:#555;';
  return `background: linear-gradient(${stops.join(', ')});`;
}
function flagBadge(teamName, size) {
  return `<span class="flag flag-${size}" style="${flagStyle(teamName)}"></span>`;
}

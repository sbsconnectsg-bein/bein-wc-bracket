...
// Trusting `winner` unconditionally (previous version of this function)
// fixed the corrupted-status case (a FINISHED match with a garbled status
// string) but introduced a worse problem: football-data.org also populates
// `winner` with whoever is CURRENTLY LEADING on an IN_PLAY match — not just
// final results. That meant a live, still-changeable match got treated as
// already decided, prematurely advancing a team before the game was over.
//
// The correct rule is neither "require status === FINISHED" (too strict,
// breaks on corruption) nor "trust winner always" (too loose, breaks on
// live matches) — it's excluding the specific statuses that mean the match
// hasn't concluded yet, and trusting winner for everything else (including
// unrecognized/corrupted status strings, which are far more likely to mean
// "finished but the status field itself got mangled" than "still playing").
const NOT_YET_DECIDED_STATUSES = new Set([
  'SCHEDULED', 'TIMED', 'IN_PLAY', 'PAUSED', 'POSTPONED', 'SUSPENDED', 'CANCELLED',
]);

function getWinnerName(m) {
  if (!m || !m.winner || m.winner === 'DRAW') return null;
  if (NOT_YET_DECIDED_STATUSES.has(m.status)) return null;
